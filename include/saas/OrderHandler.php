...
class OrderHandler
{
	...
	public function check_order_bill_no($Bill_no,$Pos_outlet_id,$Company_id,$Bill_date_time)
	{
		...
		$stmt = $this->conn->prepare("SELECT Manual_billno FROM igain_transaction WHERE Manual_billno = ? AND Seller = ? AND Company_id = ? AND Trans_date BETWEEN '".$start_date."' AND '".$end_date."' AND Trans_type = 12");
