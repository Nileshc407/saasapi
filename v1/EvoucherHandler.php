...
class EvoucherHandler
{
	...
	public function getCompanyDetails($api_key) 
	{
        ...
        $stmt = $this->conn->prepare("SELECT Company_id,Company_name,phonecode,Redemptionratio,Currency_name,Block_points_flag,Symbol_of_currency,Gifting_enviornment_flag,Gift_payment_balance from igain_company_master JOIN igain_country_master ON igain_country_master.id=igain_company_master.Country WHERE Company_id = ?");
		
        $stmt->bind_param("s", $api_key);		
        if ($stmt->execute()) 
		{
			$res = array();
            $stmt->bind_result($Company_id,$Company_name,$phonecode,$Redemptionratio,$Currency_name,$Block_points_flag,$Symbol_of_currency,$Gifting_enviornment_flag,$Gift_payment_balance);
            $stmt->fetch();
