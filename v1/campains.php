...
require_once '../include/CampaignHandler.php';
...
$app->add(new \Slim\Middleware\SessionCookie(array(
	...
	$app->get('/campains','authenticate', function() use ($app) {
			...
			$dbHandlerObj = new DbHandler();
			$dbOffersObj = new CampaignHandler();
						...
						if ($user != NULL) {
								...
								$OfferDetails = $dbOffersObj->getOffers();
									...
									while ($offrs = $OfferDetails->fetch_array()) {
										...
										if($offrs["Loyalty_at_transaction"] >0 ){
											$offers[] =  array(
												'outlet'    =>  $offrs["Last_name"].' '.$offrs["Last_name"],
												'name'    =>  $offrs["Loyalty_name"],
												'fromdate'  => $offrs["From_date"],
												'todate'  => $offrs["Till_date"],
												'onpurchase'  => $offrs["Loyalty_at_value"],
												'offer'  => $offrs["Loyalty_at_transaction"]
											);
										} else if($offrs["Loyalty_at_value"] > 0){
											$offers[] =  array(
												'outlet'    =>  $offrs["Last_name"].' '.$offrs["Last_name"],
												'name'    =>  $offrs["Loyalty_name"],
												'fromdate'  => $offrs["From_date"],
