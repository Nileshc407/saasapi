...
$app->add(new \Slim\Middleware\SessionCookie(array(
	...
	$app->get('/hobbies','authenticate', function() use ($app) {
			...
			// print_r($request_array['hobbies_data'][1]['id']);
			// die;
			
			// check for required params
            verifyRequiredParams(array('membershipid','phoneno'),$request_array);			
            $response = array();
			...
			$Todays_date = $date->format('Y-m-d');	
						...
						if ($user != NULL) {
							...
							$Membership_ID = $user['Membership_ID'];
									...
									$Memberhobbies = $dbHandlerObj->getMemberhobbies($user['id']);
									// print_r($Memberhobbies);
									$hobby=array();
									while ($hobbies = $Memberhobbies->fetch_array()) {
										// echo "---Hobbies---".$hobbies["Hobbies"]."-<br>";
										// $hobby['Hobbies'] = $hobbies["Hobbies"];
										// $hobby['Hobbie_id'] = $hobbies["Hobbie_id"];


										$hobby[] =  array(
											'id'    =>  $hobbies["Hobbie_id"],
											'name'  => $hobbies["Hobbies"]
										);
									}
									// print_r($hobby);
									$response["hobbies"] = $hobby;
									...
									/* Log Entry  */
										...
										$LogParam['Transaction_type'] = 'Update Hobbies';
	...
	$app->post('/updatehobbies','authenticate', function() use ($app) {
			...
			$Todays_date = $date->format('Y-m-d');	
						...
						if ($user != NULL) {
							...
							$id = $user['id'];
							$Membership_ID = $user['Membership_ID'];
								if(!empty($request_array['hobbies_data'])){
								
								// print_r($request_array['hobbies_data'][1]['id']);
								// print_r($request_array['hobbies_data']);
									
									foreach($request_array['hobbies_data'] as $hobby){
										 
										// echo "<br>----hobby---".$hobby['id'];	
										// echo "<br>----hobbies---".$hobby['hobbies'];	

										$HobbiPara['Hobbie_id']=$hobby['id'];
										$HobbiPara['Enrollement_id']=$user['id'];
										$HobbiPara['Company_id']=$_SESSION["company_id"];


										$alreadyExit = $dbHandlerObj->checkMemberhobbies($hobby['id'],$user['id']);
																				
										// echo "<br>----alreadyExit---".$alreadyExit;	
										if($alreadyExit==0){
											$Memberhobbies = $dbHandlerObj->insertMemberhobbies($HobbiPara);
										}
									...
									$response['errorcode'] = 3118;
									$response['message'] = "Invalid Hobbies Data";
	...
	$app->post('/deletehobbies','authenticate', function() use ($app) {
			...
			$param['phoneno'] = $_SESSION["phonecode"].''.$request_array['hobbies_data']['phoneno'];
							...
							$Membership_ID = $user['Membership_ID'];
								...
								// print_r($request_array['hobbies_data']);
									...
									foreach($request_array['hobbies_data'] as $hobby){
										...
										// echo "<br>----alreadyExit---".$alreadyExit;	
										if($alreadyExit==1){
											$Memberhobbies = $dbHandlerObj->deleteMemberhobbies($hobby['id'],$user['id']);
										}
