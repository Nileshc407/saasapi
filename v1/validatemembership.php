...
$app->add(new \Slim\Middleware\SessionCookie(array(
	...
	$app->post('/validatemembership','authenticate', function() use ($app) {
			...
			$Todays_date = $date->format('Y-m-d');	
						...
						if ($user != NULL) {
								...
								/* status,errorcode,membershipid,email.phoneno, pointbalance,redeempoints,puchaseamount,gainpoints,debitpoints,blockpoints,tier,bonuspoints 
								...
								*/
								...
								print_r($gainpoints['Total_gained_points']);
								...
								$response["pointbalance"] = $user['Current_balance']-( $user['Blocked_points']+ $user['Debit_points']);
								$response["redeempoints"] = $user['Total_reddems'];
								$response["puchaseamount"] = $user['total_purchase'];
								$response["gainpoints"] = $gainpoints['Total_gained_points'];
